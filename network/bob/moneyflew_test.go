...
func TestGetMoneyFlew_EmptyTick_NoRPC(t *testing.T) {
	...
	status, err := GetMoneyFlew(context.Background(), NewClient(srv.Client(), srv.URL), 100, []types.Transaction{})
	require.NoError(t, err)
...
func TestGetMoneyFlew_RPCError(t *testing.T) {
	...
	tx1, _ := makeTx(t, 1)
	_, err := GetMoneyFlew(context.Background(), NewClient(srv.Client(), srv.URL), 100, []types.Transaction{tx1})
	require.Error(t, err)
